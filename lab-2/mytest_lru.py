...
def switch_tests():
    ...
    s.expect(PacketOutputEvent("eth0", reqpkt, display=Ethernet), "Ethernet frame destined for 40:00:00:00:00:01 should be forwarded out eth0")
    s.expect(PacketInputTimeoutEvent(1.0), "The switch should not do anything after it sends packet out frome eth0")

    # step 3: table is full & the LRU item(2) will be instead 
    reqpkt = mk_pkt("30:00:00:00:00:06", "30:00:00:00:00:02", '192.168.6.100','192.168.2.100')
    s.expect(PacketInputEvent("eth2", reqpkt, display=Ethernet), "An Ethernet frame from 30:00:00:00:00:06 to 30:00:00:00:00:02 should arrive on eth2")
    s.expect(PacketOutputEvent("eth0", reqpkt, "eth1", reqpkt, display=Ethernet), "Ethernet frame destined for 30:00:00:00:00:02 should be flooded out")
